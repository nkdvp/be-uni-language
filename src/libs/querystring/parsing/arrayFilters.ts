...
class InFilter extends Filter {
  ...
  validate(fdCon: FieldDescriptionContainer): ValidationResult {
    return arrayFilterValidation('InFilter')(
      fdCon,
      this.supportedTypes,
      this.variable,
      this.values,
  ...
  toMySqlCriteria(fdCon: FieldDescriptionContainer): string {
    ...
    const values = this.values.map((v) => stringToData(fd.dataType)(v));
    const valuesSeparatedByComma = values.reduce(
      (res, elem, i) => (i ? `${res}, ${elem}` : elem),
      '',
    );
...
class NinFilter extends Filter {
  ...
  validate(fdCon: FieldDescriptionContainer): ValidationResult {
    return arrayFilterValidation('NinFilter')(
      fdCon,
      this.supportedTypes,
      this.variable,
      this.values,
...
class RangeFilter extends Filter {
  ...
  validate(fdCon: FieldDescriptionContainer): ValidationResult {
    return arrayFilterValidation('RangeFilter')(
      fdCon,
      this.supportedTypes,
      this.variable,
      this.values,
